...
class TradingBot:
    def __init__(self):
        ...
        self.risk_management = RiskManagement(
            ...
            risk_ratio=float(os.getenv("RISK_RATIO", 1.5))
        )
        self.symbol = os.getenv("TRADING_SYMBOL", 'BTCUSDT')
        self.quantity = float(os.getenv("TRADE_QUANTITY", 0.2))

        # Load trading parameters
        self.interval = os.getenv("TRADING_INTERVAL", '5')
    ...
    def job(self):
        ...
        rsi, macd, macd_signal, stoch_rsi, current_price = Helpers.calculate_and_print_indicators(df, self.indicators)

        trend = self.strategy.momentum_strategy(df)
        if trend:
